...
export default class ItemInfo extends Phaser.GameObjects.Container {
    constructor(item){
        ...
        this.add(new Phaser.GameObjects.Text(this.scene, -110, -35, `${item.name}`, {fontSize: 35, color: "#00ffff"}))
        this.text = new Phaser.GameObjects.Text(this.scene, -110, -5,`x:${item.gridX} y:${item.gridY}`,{fontSize: 35, color: "#00ffff"})
        this.add(this.text)
        this.depth = 10
